...
void lauchCUDAHuffmanCompress(
    ...
    uint64_t *gpuMemoryOverflowIndex, *integerOverflowIndex;
    ...
    if (integerOverflowFlag == 0) {
        ...
        // multiple run of kernel due to larger file or smaller gpu memory
            ...
            gpuMemoryOverflowIndex =
                (uint64_t *)calloc(numKernelRuns * 2, sizeof(uint64_t));
            createDataOffsetArray(
                compressedDataOffset,
                inputFileData,
                inputFileLength,
    ...
    // GPU initiation
    {
        ...
        error = cudaMalloc(
            (void **)&d_compressedDataOffset,
            (inputFileLength + 1) * sizeof(uint64_t)
        );
        if (error != cudaSuccess)
            printf("erro_2: %s\n", cudaGetErrorString(error));
        error = cudaMalloc(
            (void **)&d_huffmanDictionary,
    ...
    if (numKernelRuns == 1) {
        ...
        // integer overflow occurs
            ...
            if (error != cudaSuccess)
                printf("erro_7: %s\n", cudaGetErrorString(error));
            error = cudaMalloc(
                (void **)&d_byteCompressedDataOverflow,
                compressedDataOffset[inputFileLength] * sizeof(unsigned char)
            );
            if (error != cudaSuccess)
                printf("erro_8: %s\n", cudaGetErrorString(error));
            ...
            if (bitPaddingFlag[0] == 0) {
                ...
                unsigned char temp_compByte = inputFileData
                    [(compressedDataOffset[integerOverflowIndex[0]] / 8) - 1];
            ...
            for (i = 0; i < numKernelRuns; i++) {
                ...
                if (gpuBitPaddingFlag[i] == 0) {
                    ...
                    if (error != cudaSuccess)
                        printf("erro_12: %s\n", cudaGetErrorString(error));
                    inputFileData[pos - 1] =
                        temp_compByte | inputFileData[pos - 1];
                    pos += (compressedDataOffset
                                [gpuMemoryOverflowIndex[i * 2 + 1]] /
                            8) -
                           1;
                }
            ...
            for (i = 0; i < numKernelRuns; i++) {
                if (integerOverflowIndex[i] != 0) {
                    ...
                    if (gpuBitPaddingFlag[i] == 0) {
                        ...
                        unsigned char temp_huffmanTreeNode =
                            inputFileData[pos - 1];
                        error = cudaMemcpy(
                            &inputFileData[pos - 1],
                            d_inputFileData,
