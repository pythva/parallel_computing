...
extern "C" 
{
    ...
    // the frequency array, etc., and return a newly allocated compressed buffer plus length.
    void runCudaLand(int   myrank,
                     unsigned char  *inputData,
                     unsigned int    blockLength,
                     unsigned int   *frequency,
                     unsigned char **d_compressedData,  // [OUT] pointer to newly alloc’d CPU buffer
                     unsigned int   *compBlockLength);   // [OUT] final number of compressed bytes
}
...
void runCudaLand(int   myrank,
                 ...
                 unsigned int   *hostCompressedSize)  // [OUT]
{
    // Optional: choose a GPU if you have multiple per node
    cudaSetDevice(myrank % 4);

    // For demonstration, let’s replicate steps from your original CUDACompress main:
    ...
    //    for large input. That’s up to you. For brevity, we skip or keep a shorter version.

    // 4) We allocate an offset array for the GPU to use for bit positions, etc.
    //    In your original code, you do:
    //       compressedDataOffset = (unsigned int *)malloc((inputFileLength + 1)* ...);
    //    Just replicate as needed:

    unsigned int *compressedDataOffset = (unsigned int *)malloc((inputSize+1)*sizeof(unsigned int));

    // 5) Launch the GPU compression (like your original lauchCUDAHuffmanCompress):
    //    This function presumably does the parallel bit-packing on the GPU. 
    //    You had parameters: (inputData, offsetArray, inputSize, numKernelRuns, integerOverflowFlag, mem_req)
    //
    //    For “numKernelRuns” or “mem_req”, your old code computed them by looking at the GPU memory,
    //    or by some formula. Keep that if you want.  For brevity, we do a simpler approach:
    ...
    // 6) In your original code, the GPU kernel writes the compressed bits into the beginning
    //    of inputFileData or some device buffer. Then you figure out how many total bits/bytes
    //    got used.  In your original `CUDACompress.cu`, you eventually do something like:
    //         fwrite(&inputFileLength, ...)
    //         fwrite(frequency, ...)
    //         fwrite(inputFileData, 1, mem_offset/8, compressedFile);
    //
    //    Instead of writing to a file here, we want to return the compressed buffer in 
    ...
    for (unsigned int i = 0; i < 256; i++) {
        if (frequency[i] > 0) {
            ...
            mem_offset += ( (unsigned long)frequency[i] *
    ...
    //    which presumably your kernel wrote to `inputFileData` or some GPU buffer.
    //    In your original code, you wrote them directly from `inputFileData`.
    //    If lauchCUDAHuffmanCompress wrote them into device memory, 
    //    you should do a cudaMemcpy back to host. 
    //
    //    If your code (like the original) reuses `inputFileData` on the CPU for the compressed bits,
    //    then you can just pass them along.  Let’s assume the GPU wrote them in-place to `inputFileData`:
    //    So we just copy from that array into a new array of length = totalCompressedBytes.
    ...
    *hostCompressedSize = totalCompressedBytes;
    ...
    // (Optional) Debug
    printf("Rank %d (runCudaLand): InputSize=%u compressed to %u bytes\n",
           myrank, inputSize, totalCompressedBytes);
}
